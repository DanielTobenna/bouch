	...
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('greatestinvestapp/loginAlert2.html', {'name':username})
...
def signup(request):
	...
	if form.is_valid():
		...
		template= render_to_string('greatestinvestapp/welcomeEmail2.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
		...
		second_template= render_to_string('greatestinvestapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
...
def deposit(request):
	if request.method=='POST':
		...
		order_id= 'Bitprintinvestmentservice'
		...
		if price_amount and pay_currency:
			...
			template= render_to_string('greatestinvestapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
...
def pending_deposit(request):
	...
	return render(request, 'greatestinvestapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'greatestinvestapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'greatestinvestapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'greatestinvestapp/completed_transaction.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('greatestinvestapp/withdrawalEmail2.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('greatestinvestapp/declineWithdrawalEmail2.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def update_payment(request, pk):
	...
	template= render_to_string('greatestinvestapp/confirmDepositEmail2.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def create_bonus(request):
	...
	if form.is_valid():
		...
		template= render_to_string('greatestinvestapp/bonusNotification.html', {'message':message, 'amount':amount, 'code':code})
		plain_message= strip_tags(template)
		emailmessage= EmailMultiAlternatives(
	...
	if request.method == 'POST':
		...
		if payment_code:
			...
			if check_payment_code:
				...
				template= render_to_string('greatestinvestapp/bonusComplete.html', {'name':client_username, 'bonus_amount':bonus_amount, 'payment_code':payment_code})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
...
def meetrobert(request):
	...
	context={}
	return render(request, 'greatestinvestapp/meetrobert.html', context)
...
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('greatestinvestapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
